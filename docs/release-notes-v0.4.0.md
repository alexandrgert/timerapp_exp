...
База `data.json` сохраняется **при каждом изменении** (атомарная запись через временный файл).

**При обычном закрытии** (меню «Выход», закрытие из трея) программа ещё раз сохраняет
данные и создаёт **резервную копию с меткой времени** в каталоге `backups/` рядом с `data.json`.
Если включён WebDAV — по настройке может отправить актуальную базу на сервер.

**При аварийном завершении**, когда ОС успевает передать сигнал процессу
(`SIGTERM`, `SIGINT`, `SIGHUP` — завершение из systemd, прерывание в терминале),
срабатывает та же процедура: сохранение и резервная копия.

**Не сработает** при жёстком убийстве процесса (`kill -9`), обрыве питания или kernel OOM —
в этих случаях остаются последнее успешное сохранение и файл `data.json.bak`.

Дополнительно при каждом сохранении обновляется файл **`data.json.bak`** — предыдущая версия
базы. Если основной файл повредится, при следующем запуске программа **восстановит данные**
из `.bak` автоматически.
...
- Резервные копии при выходе хранятся в `backups/` (до 30 последних); `data.json.bak` — ещё
  один уровень защиты на случай повреждения основного файла.
- Защита при выходе **не гарантирована** при `kill -9` и внезапном отключении питания.
